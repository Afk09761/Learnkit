  ...
  setTimeout(() => {
    ...
    banner.innerHTML = `
      <div style="display:flex;align-items:center;gap:1rem;flex-wrap:wrap;">
        ...
        <button id="pwa-install-btn" style="margin-left:auto;background:var(--sage);color:white;border:none;padding:.5rem 1.1rem;border-radius:999px;font-weight:700;font-size:.82rem;white-space:nowrap;">Install Free</button>
        <button id="pwa-dismiss-btn" style="background:none;border:none;color:inherit;opacity:.5;font-size:1.1rem;padding:.2rem .4rem;line-height:1;">✕</button>
      </div>
    `;
    banner.style.cssText = 'position:fixed;bottom:1.5rem;left:50%;transform:translateX(-50%);background:var(--cream);border:1px solid var(--sand);border-radius:var(--radius-xl);padding:1rem 1.3rem;box-shadow:var(--shadow-deep);z-index:800;width:min(420px,calc(100vw - 3rem));color:var(--ink);font-family:var(--font-body);animation:slideUp .4s var(--ease-smooth) both;';
